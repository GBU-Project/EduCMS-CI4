...
<?php echo form_open_multipart('admin/partners/edit/' . $row->id); ?>
<div class="row">
    <div class="col-md-8">
        <?php echo educard_start('Detail Mitra', 'handshake'); ?>
            <?php echo eduform_input('name', 'Nama Mitra', set_value('name', $row->name ?? ''), 'text', array('required' => TRUE)); ?>
            <?php echo eduform_input('link', 'Tautan Website (Opsional)', set_value('link', $row->link ?? ''), 'url', array('placeholder' => 'https://')); ?>
    ...
    <div class="col-md-4">
        ...
        <?php echo educard_start('Logo Mitra', 'image'); ?>
            <?php echo eduform_file('logo', 'Ganti Logo', $row->logo ?? '', array('help' => 'Kosongkan jika tidak ingin mengganti.')); ?>
        <?php echo educard_end(); ?>
    </div>
</div>
<?php echo form_close(); ?>
