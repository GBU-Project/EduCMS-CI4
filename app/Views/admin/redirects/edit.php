...
<?php echo form_open('admin/redirects/edit/' . $row->id); ?>
<div class="row">
    <div class="col-md-8">
        <?php echo educard_start('Detail Redirect', 'route'); ?>
            <?php echo eduform_input('source_url', 'URL Asal', set_value('source_url', $row->source_url ?? ''), 'text', array('required' => TRUE)); ?>
            <?php echo eduform_input('target_url', 'URL Tujuan', set_value('target_url', $row->target_url ?? ''), 'text', array('required' => TRUE)); ?>
        <?php echo educard_end(); ?>
    </div>
    <div class="col-md-4">
        <?php echo educard_start('Tipe Redirect', 'paper-plane'); ?>
            <?php echo eduform_select('status_code', 'Status Code', array('301' => '301 - Permanent', '302' => '302 - Temporary'), set_value('status_code', $row->status_code ?? '301')); ?>
            <p class="text-muted small mb-0">Total hit: <strong><?php echo (int)($row->hit_count ?? 0); ?></strong></p>
            <div class="mt-4">
