...
<?php echo form_open('admin/announcements/edit/' . $row->id); ?>
<div class="row">
    <div class="col-md-8">
        <?php echo educard_start('Detail Pengumuman', 'bullhorn'); ?>
            <?php echo eduform_input('title', 'Judul Pengumuman', set_value('title', $row->title ?? ''), 'text', array('required' => TRUE)); ?>
            <?php echo eduform_input('slug', 'Slug URL', set_value('slug', $row->slug ?? ''), 'text'); ?>
            <?php echo eduform_textarea('content', 'Isi Pengumuman', set_value('content', $row->content ?? ''), array('rows' => 8)); ?>
    ...
    <div class="col-md-4">
        <?php echo educard_start('Publikasi', 'paper-plane'); ?>
            ...
            <?php echo eduform_checkbox('is_pinned', 'Sematkan pengumuman ini (tampil di atas)', set_value('is_pinned', !empty($row->is_pinned) ? '1' : '0') == '1'); ?>
            <div class="mt-4">
