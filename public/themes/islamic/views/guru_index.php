...
<main class="flex-grow w-full py-12 sm:py-16 bg-slate-50 border-b border-emerald-100">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-10">
        ...
        <nav class="flex items-center space-x-2 text-xs font-semibold text-slate-500 bg-white px-4 py-2.5 rounded-2xl border border-emerald-100/80 shadow-sm w-fit">
            ...
            <span class="text-emerald-950 font-bold">Direktori Guru</span>
        </nav>
        ...
        <?php if (empty($teachers)): ?>
            <div class="bg-white rounded-3xl border border-emerald-100 p-16 text-center shadow-sm">
                ...
                <h3 class="font-extrabold text-emerald-950 text-lg">Belum Ada Data Guru</h3>
                <p class="text-slate-500 text-sm mt-1">Data guru yang ditambahkan admin akan tampil di sini.</p>
            </div>
        <?php else: ?>
            <div class="grid sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
                <?php foreach ($teachers as $teacher): ?>
                    <a href="<?php echo base_url('guru/' . $teacher->id); ?>" class="islamic-card bg-white rounded-3xl overflow-hidden border border-emerald-100 shadow-sm hover:shadow-md transition text-center group flex flex-col">
                        <div class="h-48 bg-emerald-900 relative overflow-hidden">
                            <?php if (!empty($teacher->photo)): ?>
                                <img src="<?php echo base_url($teacher->photo); ?>" alt="<?php echo esc_attr($teacher->name); ?>" class="w-full h-full object-cover group-hover:scale-105 transition duration-500">
                            <?php else: ?>
                        ...
                        <div class="p-5 flex-grow flex flex-col justify-center space-y-1">
                            <h3 class="font-extrabold text-base text-emerald-950 group-hover:text-emerald-700 transition leading-snug"><?php echo esc_html($teacher->name); ?></h3>
                            <p class="text-xs text-slate-500 font-medium"><?php echo esc_html($teacher->position); ?></p>
                        </div>
                    </a>
                <?php endforeach; ?>
            </div>
