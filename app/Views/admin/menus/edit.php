                ...
                <div class="card card-premium shadow-sm">
                    <div class="card-header bg-white border-0 py-3">
                        <h5 class="card-title font-weight-bold mb-0 text-dark">
                            <?php echo render_icon('edit', 'text-indigo mr-2'); ?>
                            Form Edit Item Navigasi
                        </h5>
                    </div>
                    <div class="card-body">
                        
                        <form action="<?php echo base_url('admin/menus/edit/' . $row->id); ?>" method="post" id="menu-form">
                            <input type="hidden" name="group_id" value="<?php echo $group_id; ?>">
                            <input type="hidden" name="internal_url" id="internal_url" value="<?php echo esc_attr($row->url); ?>">

                            <!-- Title -->
                            <?php echo eduform_input('title', 'Judul Menu', set_value('title', $row->title), 'text', array('required' => TRUE, 'placeholder' => 'Contoh: Profil Sekolah')); ?>
                            ...
                            <div class="form-group mb-3">
                                ...
                                <select name="parent_id" id="parent_id" class="form-control custom-select">
                                    ...
                                    <?php foreach ($parents as $p): ?>
                                        <option value="<?php echo $p->id; ?>" <?php echo set_select('parent_id', $p->id, ($row->parent_id == $p->id)); ?>>
                            ...
                            <div class="row">
                                <div class="col-md-6">
                                    <?php echo eduform_input('icon', 'Icon (FontAwesome Class)', set_value('icon', $row->icon), 'text', array('placeholder' => 'Contoh: house, user, link', 'help' => 'Nama icon FontAwesome v6 (tanpa fa- prefix).')); ?>
                                </div>
                                <div class="col-md-6">
                                    <?php echo eduform_input('description', 'Deskripsi Singkat', set_value('description', $row->description), 'text', array('placeholder' => 'Keterangan menu', 'help' => 'Opsional: Penjelasan singkat fungsi menu.')); ?>
                                </div>
                            </div>
                            ...
                            <?php echo eduform_input('url', 'URL Tujuan', set_value('url', $row->url), 'text', array('required' => TRUE, 'placeholder' => 'Contoh: /page/profil atau https://google.com', 'readonly' => TRUE, 'help' => 'URL navigasi link menu.')); ?>

                            <div class="row">
                                <div class="col-md-6">
                                    <!-- Target -->
                                    <?php echo eduform_select('target', 'Buka Link Di', array('_self' => 'Tab yang Sama (Default)', '_blank' => 'Tab Baru (Open in New Tab)'), set_value('target', $row->target)); ?>
                                </div>
                                <div class="col-md-6">
                                    <!-- Status -->
                                    <?php echo eduform_select('status', 'Status Publikasi', array('published' => 'Aktif (Diterbitkan)', 'draft' => 'Draft (Diarsipkan)'), set_value('status', $row->status)); ?>
                                </div>
                            </div>

                            <!-- Sort Order -->
                            <?php echo eduform_input('order_num', 'Sort Order (No Urutan)', set_value('order_num', $row->order_num), 'number', array('required' => TRUE, 'placeholder' => '1', 'help' => 'Angka urutan tampilan menu.')); ?>

                            <!-- Form Actions -->
                            <div class="mt-4 pt-3 border-t">
                                <button type="submit" class="btn btn-sm btn-indigo mr-2">
                                    <?php echo render_icon('save', 'mr-1'); ?> Perbarui Item Menu
                                </button>
...
<script>
    ...
    $(document).ready(function() {
        ...
        // =========================================================
        // PARSE STABLISHED DATABASE URL AND INITIALIZE FORM
        // =========================================================
        const existingUrl = "<?php echo addslashes($row->url); ?>";
        const isExternal = existingUrl.match(/^(https?:)?\/\//i) || existingUrl.startsWith('mailto:') || existingUrl.startsWith('tel:') || existingUrl.startsWith('#');
        
        if (isExternal) {
            $('#link_type_external').prop('checked', true).trigger('change');
            $('#url').val(existingUrl);
        } else {
            $('#link_type_internal').prop('checked', true).trigger('change');
            
            let intType = '';
            let slug = '';
            
            if (existingUrl.startsWith('/page/')) {
                intType = 'pages';
                slug = existingUrl.replace('/page/', '');
            } else if (existingUrl.startsWith('/berita/')) {
                intType = 'posts';
                slug = existingUrl.replace('/berita/', '');
            } else if (existingUrl.startsWith('/berita?category=')) {
                intType = 'categories';
                slug = existingUrl.replace('/berita?category=', '');
            } else if (existingUrl.startsWith('/berita?tag=')) {
                intType = 'tags';
                slug = existingUrl.replace('/berita?tag=', '');
            } else if (existingUrl.startsWith('/prestasi/')) {
                intType = 'achievements';
                slug = existingUrl.replace('/prestasi/', '');
            } else if (existingUrl.startsWith('/agenda/')) {
                intType = 'agendas';
                slug = existingUrl.replace('/agenda/', '');
            } else if (existingUrl.startsWith('/ekstrakurikuler/')) {
                intType = 'extracurriculars';
                slug = existingUrl.replace('/ekstrakurikuler/', '');
            } else if (existingUrl === '/posts' || existingUrl === '/berita') {
                intType = 'news_index';
            } else if (existingUrl === '/ppdb') {
                intType = 'ppdb';
            } else if (existingUrl === '/contact') {
                intType = 'contact';
            } else if (existingUrl === '/galeri-foto') {
                intType = 'gallery_photo';
            } else if (existingUrl === '/galeri-video') {
                intType = 'gallery_video';
            } else if (existingUrl === '/download') {
                intType = 'download';
            } else if (existingUrl === '/guru') {
                intType = 'guru';
            } else if (existingUrl === '/staff') {
                intType = 'staff';
            }
            
            if (intType) {
                $('#internal_type').val(intType).trigger('change');
                if (slug) {
                    $('#internal_value').val(slug).trigger('change');
                }
                // Explicitly restore values to prevent select event overrides
                $('#url').val(existingUrl);
                $('#internal_url').val(existingUrl);
            }
        }
    });
</script>
