...
<main class="flex-grow w-full py-12 sm:py-16 bg-slate-50 border-b border-emerald-100">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-10">
        ...
        <nav class="flex items-center space-x-2 text-xs font-semibold text-slate-500 bg-white px-4 py-2.5 rounded-2xl border border-emerald-100/80 shadow-sm w-fit">
            ...
            <span class="text-emerald-950 font-bold">Video</span>
        </nav>
        ...
        <?php if (empty($videos)): ?>
            <div class="bg-white rounded-3xl border border-emerald-100 p-16 text-center shadow-sm">
                ...
                <h3 class="font-extrabold text-emerald-950 text-lg">Belum Ada Video</h3>
        ...
        <?php else: ?>
            ...
            <div class="flex justify-center items-center space-x-2 pt-8 border-t border-emerald-100">
                <?php for ($i = 1; $i <= $total_pages; $i++): ?>
                    <a href="<?php echo base_url('video?page=' . $i); ?>" class="w-9 h-9 flex items-center justify-center rounded-xl text-xs font-bold transition <?php echo $i === $current_page ? 'bg-emerald-700 text-white shadow-md' : 'bg-white text-emerald-900 border border-emerald-200 hover:bg-emerald-50'; ?>"><?php echo $i; ?></a>
                <?php endfor; ?>
            </div>
            <?php endif; ?>
        <?php endif; ?>
