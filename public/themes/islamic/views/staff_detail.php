...
<main class="flex-grow w-full py-12 sm:py-16 bg-slate-50 border-b border-emerald-100">
    <div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 space-y-8">
        ...
        <nav class="flex items-center space-x-2 text-xs font-semibold text-slate-500 bg-white px-4 py-2.5 rounded-2xl border border-emerald-100/80 shadow-sm w-fit">
            ...
            <a href="<?php echo base_url('staff'); ?>" class="hover:text-emerald-700 transition">Direktori Staf</a>
            <i data-lucide="chevron-right" class="w-3.5 h-3.5 text-emerald-500"></i>
            <span class="text-emerald-950 font-bold truncate max-w-xs"><?php echo esc_html($staff->name); ?></span>
        </nav>
        ...
        <div class="bg-white rounded-3xl p-8 sm:p-10 border border-emerald-100 shadow-md flex flex-col sm:flex-row gap-8 items-center sm:items-start text-center sm:text-left">
            <div class="w-36 h-36 rounded-3xl overflow-hidden bg-emerald-900 shrink-0 border-4 border-emerald-50 shadow-md relative">
                <?php if (!empty($staff->photo)): ?>
                    <img src="<?php echo base_url($staff->photo); ?>" alt="<?php echo esc_attr($staff->name); ?>" class="w-full h-full object-cover">
                <?php else: ?>
            ...
            <div class="space-y-3 flex-grow">
                ...
                <h1 class="text-3xl font-extrabold text-emerald-950 leading-tight"><?php echo esc_html($staff->name); ?></h1>
                <p class="text-amber-600 font-bold text-sm"><?php echo esc_html($staff->position); ?></p>
                
                <div class="pt-2 space-y-1.5 text-xs font-semibold text-slate-600">
                    <?php if (!empty($staff->email)): ?>
                        <p class="flex items-center justify-center sm:justify-start gap-2"><i data-lucide="mail" class="w-4 h-4 text-emerald-600"></i><span><?php echo esc_html($staff->email); ?></span></p>
                    <?php endif; ?>
                    <?php if (!empty($staff->phone)): ?>
                        <p class="flex items-center justify-center sm:justify-start gap-2"><i data-lucide="phone" class="w-4 h-4 text-emerald-600"></i><span><?php echo esc_html($staff->phone); ?></span></p>
                    <?php endif; ?>
                </div>
            </div>
        </div>
        ...
        <div class="pt-2">
            <a href="<?php echo base_url('staff'); ?>" class="inline-flex items-center space-x-2 text-xs font-bold text-emerald-700 hover:text-emerald-800 bg-white px-5 py-3 rounded-2xl border border-emerald-200 shadow-sm transition">
                <i data-lucide="arrow-left" class="w-4 h-4"></i>
                <span>Kembali ke Direktori Staf</span>
            </a>
        </div>
